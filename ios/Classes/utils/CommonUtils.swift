...
public class CommonUtils{
	...
	*/
	...
	static func txf_log_info(file: String = #file,
							 ...
							 content: String) {
		txf_log_swift(TXE_LOG_INFO, file.cString(using: .utf8), Int32(line), function.cString(using: .utf8), content)
	}
	...
	static func txf_log_error(file: String = #file,
							  ...
							  function: String = #function,
							  content: String) {
		txf_log_swift(TXE_LOG_ERROR, file.cString(using: .utf8), Int32(line), function.cString(using: .utf8), content)
	}
	...
	static func logFlutterMethodCall(_ call: FlutterMethodCall) {
		...
		if let args = call.arguments as? Dictionary<String, Any> {
			...
			if argsInfo.count > 0 {
				txf_log_info(content: "method=\(call.method)|arguments: {\(argsInfo)}")
			} else {
				noAgrs()
			}
		} else {
	...
	static func logError(call: FlutterMethodCall, errCode: Int, errMsg: String) {
		...
		if let args = call.arguments as? Dictionary<String, Any> {
			...
			if argsInfo.count > 0 {
				txf_log_error(content: "method=\(call.method)|arguments={\(argsInfo)}|error={errCode: \(errCode), errMsg: \(errMsg)}")
			} else {
				noAgrs()
			}
		} else {
